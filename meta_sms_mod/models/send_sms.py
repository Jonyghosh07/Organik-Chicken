...
class SendSMS(models.TransientModel):
    ...
    def send_sms(self, number, sms_text):
        ...
        sms_provider = self.env['ir.default'].sudo().get('res.config.settings', 'sms_provider')

        if sms_provider == 'elitbuzz':
            api_token = self.env['ir.default'].sudo().get('res.config.settings', 'elitbuzz_api_token')
            sid = self.env['ir.default'].sudo().get('res.config.settings', 'elitbuzz_sid')            
            ...
            _logger.info(
                        "Send sms with ElitBuzz ---------->%s",
                        pprint.pformat(r.content))
            params_type = "success"
            ...
            raise ValidationError("Please Select A SMS Provider First And Give It's API Token/Key")
